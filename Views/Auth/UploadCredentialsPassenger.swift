...
struct UploadCredentialsPassenger: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showProfileImagePicker) {
            ImagePickerPassenger(image: $profilePhoto, sourceType: imageSourceType)
        }
        .sheet(isPresented: $showLicenseImagePicker) {
            ImagePickerPassenger(image: $driversLicense, sourceType: imageSourceType)
        }
    }
    ...
    private var infoSection: some View {
        VStack(spacing: 8) {
            InfoRowPassenger(text: "Clear, well-lit photos work best")
            InfoRowPassenger(text: "Ensure all details are visible")
        }
    ...
    private func confirmCredentials() {
        print("Passenger Profile Photo uploaded: \(profilePhoto != nil)")
        print("Passenger Driver's License uploaded: \(driversLicense != nil)")
        navigateToHome = true
    }
}

// MARK: - Upload Photo Button Component
struct UploadPhotoButtonPassenger: View {
    let image: UIImage?
    let icon: String
    let label: String
    let action: () -> Void
...
// MARK: - Image Picker for Passenger
struct ImagePickerPassenger: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        let parent: ImagePickerPassenger
        
        init(_ parent: ImagePickerPassenger) {
            self.parent = parent
        }
