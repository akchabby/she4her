...
struct SafetyPageDriver: View {
    ...
    @State private var driverCoordinate = CLLocationCoordinate2D(latitude: 37.3349, longitude: -122.0090)
    @State private var passengerCoordinate = CLLocationCoordinate2D(latitude: 37.3317, longitude: -122.0301)
    ...
    var body: some View {
        ZStack {
            ...
            Map(position: $cameraPosition) {
                Marker("You", coordinate: driverCoordinate)
    ...
    private func calculateRoute() {
        let center = CLLocationCoordinate2D(
            latitude: (driverCoordinate.latitude + passengerCoordinate.latitude) / 2,
            longitude: (driverCoordinate.longitude + passengerCoordinate.longitude) / 2
        )
...
struct SafetyActionButtonDriver: View {
    let icon: String
    let title: String
    let subtitle: String
    let iconColor: Color
    ...
    var body: some View {
        Button(action: action) {
            SafetyActionButtonViewDriver(icon: icon, title: title, subtitle: subtitle, iconColor: iconColor)
        }
    }
}

struct SafetyActionButtonViewDriver: View {
    let icon: String
    let title: String
    let subtitle: String
